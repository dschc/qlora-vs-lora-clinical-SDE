...
GPU_ID = args.gpu_id
BERT_GPU_ID = args.bert_gpu_id if len(args.bert_gpu_id) > 1 else args.bert_gpu_id[0]

# Prepare output directory and filename
output_dir = os.path.dirname(TEST_FILE_PATH)
test_filename = os.path.basename(TEST_FILE_PATH)
base_filename = os.path.splitext(test_filename)[0]
...
print(f"Loading test results from: {TEST_FILE_PATH}")
data = json.load(open(TEST_FILE_PATH, mode="r", encoding="utf-8"))
print(f"Loaded {len(data)} examples")

# Load SciSpacy model for clinical NER and Initialize UMLS Entity Linker
print(f"Loading SciSpacy model on GPU {GPU_ID}...")
...
for example in tqdm(data):
    ...
    for field in fields:
        ...
        if field in fields_with_medical_entities:
            ...
            # Save the entities for further study    
...
# Save entities comparison 
entity_comparison_file_name = os.path.join(output_dir, f"{base_filename}_entity_comparison.json")
with open(entity_comparison_file_name, mode="w", encoding="utf-8") as f:
    json.dump(entity_comparison_data, f, indent=4)

del entity_comparison_data

print(f"Entity comparison saved to: {entity_comparison_file_name}")
...
for field in tqdm(fields):
    ...
    if refs and preds:
        ...
        bert_metrics[field] = np.array([])



# Save results
results_file_name = os.path.join(output_dir, f"{base_filename}_results.txt")
print(f"\nSaving results to: {results_file_name}")

with open(results_file_name, mode="w", encoding="utf-8") as results_file:
    results_file.write(f"{'='*60}\n")
    results_file.write(f"Evaluation Results\n")
    results_file.write(f"{'='*60}\n")
    results_file.write(f"Test File: {TEST_FILE_PATH}\n")
    results_file.write(f"Task: {TASK}\n")
    results_file.write(f"Model: {MODEL_NAME}\n")
    results_file.write(f"Language: {LANG}\n")
    results_file.write(f"{'='*60}\n\n")
    
    results_file.write("=== ROUGE Scores ===\n")
    for field in fields:
        rouge1 = np.mean(rouge_metrics[field]['rouge1']) if rouge_metrics[field]['rouge1'] else 0
        rouge2 = np.mean(rouge_metrics[field]['rouge2']) if rouge_metrics[field]['rouge2'] else 0
        rougeL = np.mean(rouge_metrics[field]['rougeL']) if rouge_metrics[field]['rougeL'] else 0
        results_file.write(f"{field}: ROUGE-1={rouge1:.4f}, ROUGE-2={rouge2:.4f}, ROUGE-L={rougeL:.4f}\n")
    results_file.write("\n")

    results_file.write("=== BERTScore ===\n")
    for field in fields:
        f1 = np.mean(bert_metrics[field]) if len(bert_metrics[field]) > 0 else 0
    ...
    for field in fields_with_medical_entities:
        precision = np.mean(entity_metrics[field]['precision']) if entity_metrics[field]['precision'] else 0
        recall = np.mean(entity_metrics[field]['recall']) if entity_metrics[field]['recall'] else 0
        f1 = np.mean(entity_metrics[field]['f1']) if entity_metrics[field]['f1'] else 0
